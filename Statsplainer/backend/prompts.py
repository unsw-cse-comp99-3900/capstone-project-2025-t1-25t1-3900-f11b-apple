def prompt_builder(mode):
    if mode == "Definition":
        ...
        return """
                ...
                Here is an exemplar of the type of response I want from you:
                Let’s say a study finds that Drug A reduces the risk of stroke by 30% (RR = 0.70, 95% CI: 0.55–0.89, p = 0.004). Think of it like this: if your chance of stroke is like drawing a losing card from a deck of 100, and normally 7 people draw that card, Drug A lowers that number to about 5. That’s a small shift — a 2.1% absolute difference — but across a crowd, it adds up. Out of every 48 people who take the drug for 5 years, one stroke is prevented. That’s like needing 48 umbrellas in a storm to keep one person completely dry — not perfect, but still protection.

                The confidence interval is like weather forecasts saying the rain will drop by 11–45%. We’re not sure of the exact number, but we’re confident it’s going down. And the p-value of 0.004? That’s like saying there’s only a 0.4% chance the clear skies happened by coincidence — the umbrella likely worked.

                So when we say the effect is “probably real,” we mean this isn’t a fluke. It’s like seeing fewer puddles after everyone’s been handed umbrellas — not magic, but meaningful.
            """
    else:
